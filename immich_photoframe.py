...
OVERLAY_OPACITY     = 20            # 0 (invisible) – 255 (fully opaque)
TEXT_OPACITY        = 120           # 0 (invisible) – 255 (fully opaque)
FONT_SIZE_DIVISOR   = 50            # larger = smaller. font size = image width ÷ this value (~40px on 1080p)
...
def add_date_overlay(image_bytes, date_str):
    """
    ...
    font_size = max(12, w // FONT_SIZE_DIVISOR)
    margin    = max(8,  w // MARGIN_DIVISOR)
...
class Handler(BaseHTTPRequestHandler):
    def log_message(self, fmt, *args):
        ...
        print(f"[immich-photoframe] {self.address_string()} {fmt % args}")
    ...
    def do_GET(self):
        ...
        try:
            ...
            if date_str:
                image_bytes = add_date_overlay(image_bytes, date_str)
            ...
            print(f"[immich-photoframe] ERROR: {e}")
...
if __name__ == "__main__":
    print(f"[immich-photoframe] Listening on port {PORT}")
    print(f"[immich-photoframe]  /horizontal → album {HORIZONTAL_ALBUM_ID}")
    print(f"[immich-photoframe]  /vertical   → album {VERTICAL_ALBUM_ID}")
